...
def polt_elemTable(plot_data):
    ...
    # 获取YlOrRd颜色条
    my_cmap = cm.get_cmap('bwr') # 'Wistia', bwr
    # 将plot_data数据映射为颜色，根据实际情况调整
    norm = mpl.colors.Normalize(-1, 1)
    # 设置超出颜色条下界限的颜色（None为不设置，即白色）
    # my_cmap.set_under('None')
    # my_cmap.set_over('None')
    # 关联颜色条和映射
    cmmapable = cm.ScalarMappable(norm, my_cmap)
    # 绘制颜色条
    plt.colorbar(cmmapable, drawedges=False, orientation='horizontal',fraction=0.048, pad=0.0, aspect=50, extend='max')
    ...
    for e in elements:
        ...
        # ele_count = round(float(ele_count), 2)
        # 在cell中添加原子序数属性
